...
let activeScan = null; // V13.3AA: (War gelöscht)
let gattServer = null; // V13.3AA: (War gelöscht)
let gattCharacteristicMap = new Map(); // V13.3AA: (War gelöscht)
let appCallbacks = {}; // V13.3AA: (War gelöscht)
...
export function initBluetooth(callbacks) {
    appCallbacks = callbacks; // V13.3AA: (Funktioniert jetzt)
    deviceMap.clear();
    gattCharacteristicMap.clear();
...
/**
 * V13.3Z: (unverändert)
 * V13.3AA: (Funktioniert jetzt, da 'activeScan' (V13.3AA) existiert)
 */
export async function startScan() {
    ...
    try {
        ...
        setScanStatus(false);
        activeScan = null; // V13.3AA: (Funktioniert jetzt)
        return false; // V12.1
    }
}
